...
from AyuXmusic.misc import HAPP, SUDOERS, XCB
from AyuXmusic.utils.database import (
    get_active_chats,
    remove_active_chat,
    remove_active_video_chat,
)
from AyuXmusic.utils.decorators.language import language
...
def _is_current_project_process(proc) -> bool:
    ...
    markers = ("watchdog.py", "ayuxmusic", "bash start")
...
async def update_(client, message, _):
    ...
    to_exc = ["git", "fetch", "origin", str(config.UPSTREAM_BRANCH)]
    _run_quiet(to_exc)
    ...
    if len(_final_updates_) > 4096:
        url = await SidduBin(updates)
        nrs = await response.edit(
            f"<b>ᴀ ɴᴇᴡ ᴜᴩᴅᴀᴛᴇ ɪs ᴀᴠᴀɪʟᴀʙʟᴇ ғᴏʀ ᴛʜᴇ ʙᴏᴛ !</b>\n\n➣ ᴩᴜsʜɪɴɢ ᴜᴩᴅᴀᴛᴇs ɴᴏᴡ\n\n<u><b>ᴜᴩᴅᴀᴛᴇs :</b></u>\n\n<a href={url}>ᴄʜᴇᴄᴋ ᴜᴩᴅᴀᴛᴇs</a>"
        )
    else:
        nrs = await response.edit(_final_updates_, disable_web_page_preview=True)
    stash = _run_quiet(["git", "stash"])
    ...
    if await is_heroku():
        ...
        _run(["pip3", "install", "-r", "requirements.txt"])
        _spawn_restart()
        exit()
